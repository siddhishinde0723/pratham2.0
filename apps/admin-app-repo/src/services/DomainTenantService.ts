...
export const findTenantByDomain = (
  ...
  // HARDCODED FOR TESTING: Force swadhaar-admin domain
  // Uncomment the line below to test with Swadhaar domain
  // let domain = 'swadhaar-admin.sunbirdsaas.com';
  
  // Fallback to actual domain if not provided
  if (!domain) {
  ...
  console.log('🔧 [DEV MODE] Using domain:', domain);

  if (!domain) {
    console.warn('[TenantService] No domain provided and window is not available');
    return null;
  }
  ...
  const matchedTenant = tenants.find((t) => {
    ...
    // Also try matching without hyphens (swadhaar-admin -> swadha ar)
    const tenantKeyWithoutHyphen = tenantKey.replace(/-/g, '');
    const tenantNameWithoutHyphen = tenantName?.replace(/-/g, '');

    return (
      tenantDomainKey === tenantKey ||
      tenantDomainKey === sanitizedDomain ||
      tenantName === tenantKey ||
      // Match with hyphen variants
      tenantName === tenantKeyWithoutHyphen ||
      tenantNameWithoutHyphen === tenantKey ||
      // Check if tenant key starts with tenant name (swadhaar-admin starts with swadhaar)
      tenantKey.startsWith(tenantName || '') ||
      (tenantName && tenantKey.includes(tenantName))
    );
  });
 ...
 */
/**
 * Checks if the current channel/tenant is Swadhaar
 * Uses the same logic as login.tsx - checks domain, tenant name, and current URL
 */
export const isSwadhaarChannel = (): boolean => {
  if (typeof window === 'undefined') {
    return false;
  }
  ...
  try {
    ...
    if (tenantDataStr) {
      try {
        ...
        if (tenantDomain.includes('swadhaar') || tenantName.includes('swadhaar')) {
          return true;
        }
      } catch (e) {
    ...
    console.error('Error checking Swadhaar channel:', error);
    return false;
  }
};
...
export const getLocalizedText = (
  ...
  if (typeof text === "object") {
    ...
    // Fallback to English
    if (text["en"] || text["EN"]) {
      return text["en"] || text["EN"] || fallback;
    }
    
    // Fallback to first available language
    const firstKey = Object.keys(text)[0];
    if (firstKey && text[firstKey]) {
