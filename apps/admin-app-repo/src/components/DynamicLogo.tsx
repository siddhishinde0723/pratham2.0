...
const DynamicLogo: React.FC<DynamicLogoProps> = ({
  ...
  useEffect(() => {
    const fetchLogoConfig = async () => {
      try {
        ...
        console.log('DynamicLogo: Logo config:', config.LOGO_CONFIG);
  ...
  const getLogoSrc = (): string => {
    console.log('DynamicLogo: Getting logo source for type:', type);
    console.log('DynamicLogo: Error state:', error);
    console.log('DynamicLogo: Logo config:', logoConfig);
    
    if (error || !logoConfig) {
      const fallback = fallbackSrc || getDefaultLogoSrc();
      console.log('DynamicLogo: Using fallback:', fallback);
      return fallback;
    }
