...
const BulkUpload: React.FC = () => {
  ...
  const validateRecord = (row: UploadedData, index: number): string[] => {
    ...
    if (!subCategory)
      messages.push(`Row ${index + 1}: Missing Sub Category (sub_category).`);
    ...
    try {
      ...
      for (let i = 0; i < uploadedData.length; i++) {
        ...
        try {
          ...
          if (
            !record.cont_title?.trim() ||
            !record.cont_dwurl?.trim() ||
            !record.topic?.trim() ||
            !record.sub_category?.trim()
          ) {
            throw new Error(
              'Missing required fields: Title, Topic, Sub Category and Download URL.'
            );
          }
        ...
        <Box sx={{ p: 3 }}>
          ...
          >
            ...
            <Alert severity="info" sx={{ mb: 3 }}>
              Required columns: <b>topic</b>, <b>sub_category</b>,
              <b>cont_tagwords</b>,<b>cont_description cont_title</b>,
              <b>language</b>,<b>resourse_type</b> <b>author</b>,
              <b>publisher</b>,<b>year</b>,<b>cont_url</b> <b>cont_dwurl</b>.
              Allowed file types: {SUPPORTED_FILE_TYPES.join(', ')} or a public
